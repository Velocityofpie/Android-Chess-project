...
public class King extends Piece{
     ...
     */
        public boolean canMoveOut(Square startSq, Board b, Chess.Turn turn, int x1, int y1, boolean underCheck) {
            ...
            Board testBoard = b;
            Square initialSq = new Square(startSq.piece, startSq.player);

            //move king in all directions and call checkingforCheck
            if (row != 7) {
                //check down
                //move down 1
                boolean is = isMoveValid(startSq, testBoard.square[row+1][col], testBoard, turn, x1, y1, col+1, 8-row-1, false);
                Square desiredSq = null;
                if (testBoard.square[row+1][col] != null) {
                    desiredSq = new Square(testBoard.square[row + 1][col].piece, testBoard.square[row + 1][col].player);
                }
                if (is) {
                    testBoard.square[row + 1][col] = startSq;
                    testBoard.square[row][col] = null;
                    boolean stillCheck = Chess.CheckingForcheck(testBoard, row + 1, col, turn);

                    //move pieces back
                    testBoard.square[row][col] = initialSq;
                    testBoard.square[row+1][col] = desiredSq;
            ...
            if (col != 0) {
                //check right
                //move down 1
                boolean is = isMoveValid(startSq, testBoard.square[row][col-1], testBoard, turn, x1, y1, col, 8-row, false);
                Square desiredSq = null;
                if (testBoard.square[row][col-1] != null) {
                    desiredSq = new Square(testBoard.square[row][col - 1].piece, testBoard.square[row][col - 1].player);
                }

                if (is) {
                    testBoard.square[row][col - 1] = startSq;
                    testBoard.square[row][col] = null;
                    boolean stillCheck = Chess.CheckingForcheck(testBoard, row, col - 1, turn);

                    //move pieces back
                    testBoard.square[row][col] = initialSq;
                    testBoard.square[row][col-1] = desiredSq;
            ...
            if (col != 7) {
                //check left
                //move left 1
                boolean is = isMoveValid(startSq, testBoard.square[row][col+1], testBoard, turn, x1, y1, col+1+1, 8-row, false);
                Square desiredSq = null;
                if (testBoard.square[row][col+1] != null) {
                    desiredSq = new Square(testBoard.square[row][col + 1].piece, testBoard.square[row][col + 1].player);
                }
            ...
            if (row != 0 && col != 7) {
                //if not in top-right
                //check up right
                boolean is = isMoveValid(startSq, testBoard.square[row-1][col+1], testBoard, turn, x1, y1, col+1+1, 8-row+1, false);
                Square desiredSq = null;
                if (testBoard.square[row-1][col+1] != null) {
                    desiredSq = new Square(testBoard.square[row - 1][col + 1].piece, testBoard.square[row - 1][col + 1].player);
                }

                if (is) {
                    testBoard.square[row - 1][col + 1] = startSq;
                    testBoard.square[row][col] = null;
                    boolean stillCheck = Chess.CheckingForcheck(testBoard, row-1, col+1, turn);

                    //move pieces back
                    testBoard.square[row][col] = initialSq;
                    testBoard.square[row - 1][col + 1] = desiredSq;
            ...
            if (row != 0 && col != 0) {
                //if not in top-left
                //check up left
                boolean is = isMoveValid(startSq, testBoard.square[row-1][col-1], testBoard, turn, x1, y1, col, 8-row+1, false);
                Square desiredSq = null;
                if (testBoard.square[row-1][col-1] != null) {
                    desiredSq = new Square(testBoard.square[row-1][col-1].piece, testBoard.square[row-1][col-1].player);
                }
